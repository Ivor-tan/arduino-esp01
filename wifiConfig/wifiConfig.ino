...
struct config_type
{
  char sta_TOPIC[32]; // 定义得到的主题(最大32字节)
  char sta_UID[64];   // 定义配网得到的UID(最大64字节)
};
...
<body align='center' style='background-color:#F5FFFA;color:blue;text-align:center;font-size:21px;'  >\r\n\
  <form align='center'  name='input' action='/' method='POST'>\r\n\   
        ...
        UID:<br>\r\n\
        <input style='width:200px;height=25px;font-size:20px'  type='text' name='uid'><br>\r\n\
        TOPIC:<br>\r\n\
        <input style='width:200px;height=25px;font-size:20px' type='text' name='topic'><br>\r\n\
...
{ // Post回调函数
  ...
  if (server.hasArg("uid"))
  {
    ...
    strcpy(config.sta_UID, server.arg("uid").c_str()); // 复制UID
    Serial.println(config.sta_UID);
  }
  else
  {
    Serial.println("error, not found uid");
    server.send(200, "text/html", "<meta charset='UTF-8'>error, not found password");
    return;
  }
  // TOPIC 获取
  if (server.hasArg("topic"))
  {
    ...
    strcpy(config.sta_TOPIC, server.arg("topic").c_str()); // 复制TOPIC
    Serial.println(config.sta_TOPIC);
  }
  else
  {
    Serial.println("error, not found topic");
    server.send(200, "text/html", "<meta charset='UTF-8'>error, not found password");
    return;
  }
